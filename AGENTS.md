...
The Development Journal is a bounded Atlas editorial surface. Recurring machine-maintained entries must go through `python -m journal_maintainer` as documented in [docs/JOURNAL_MAINTAINER.md](docs/JOURNAL_MAINTAINER.md) and [docs/JOURNAL_MAINTAINER_IMPLEMENTATION.md](docs/JOURNAL_MAINTAINER_IMPLEMENTATION.md). Routine journal publication may not rewrite specifications, sibling repositories, or unrelated Atlas files. Journal prose is not project authority.
